...
# Avalia a escala final gerada comparando com a escala mínima por período e lhe atribui uma pontuação
# Para cada período corretamente atendido (M ou N) por dia, adiciona 0.5 pontos ao score final.
def avaliar_resultado_final(df_resultado_final, df_escala_setor_periodo, nom_setor, domingo_dias_mes_vigente):
    ...
    # Inicializa a varável do score da solução final
    score = 0

    # Inicializa a variável que obtêm o total de dias do mês em análise
    total_dias = len(df_resultado_final.columns[df_resultado_final.columns.get_loc('Setor') + 1:])

    # Inicia o loop que verifica o score de cada dia
    # Começa na coluna 2 porque as colunas 0 e 1 são funcionário e setor
    for coluna in df_resultado_final.columns[2:]:
        total_M = (df_resultado_final[coluna] == 'M').sum()
        total_N = (df_resultado_final[coluna] == 'N').sum()

        # Verifica se o dia em análise é domingo ou dia da semana para selecionar a escala correta
        if coluna in domingo_dias_mes_vigente:
        ...
        valor_M = escala_setor_selecionada.iloc[0, 2]
        valor_N = escala_setor_selecionada.iloc[0, 3]

        # Verifica se atingiu o valor mínimo em cada periodo do dia em análise
        if total_M >= valor_M:
            score += 0.5
        if total_N >= valor_N:
            score += 0.5
        
    # Calcula a porcentagem de acerto da solução
    score = (score / total_dias) * 100

    score = round(score, 2)

    # Retorna o score da solução final
    return score
